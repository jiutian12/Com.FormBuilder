    ...
    $.leeUI.controls.Grid.leeExtend($.leeUI.core.UIComponent, {
        ...
        _setHeight: function (h) {
            ...
            if (h > 0) {
                ...
                var gridHeaderHeight = p.headerRowHeight * (g._columnMaxLevel - 1) + p.headerRowHeight;
 ...
 */
jQuery.fn.leePager = function(maxentries, opts) {
	opts = jQuery.extend({
		items_per_page: 10,
...
(function($) {
	...
	var CLS_ICON_OPEN = "lee-ion-android-arrow-dropdown";
	var CLS_ICON_COLLAPSE = "lee-ion-android-arrow-dropright";

	$.fn.leeTreeListView = function(options) {
		return $.leeUI.run.call(this, "leeUITreeListView", arguments);
	};

	$.leeUIDefaults.TreeListView = {
		data: [],
		columns: [],
		showIcon: true,
		parentIcon: "x-folder",
	...
	$.leeUI.controls.TreeListView = function(element, options) {
		$.leeUI.controls.TreeListView.base.constructor.call(this, element, options);
	};
	$.leeUI.controls.TreeListView.leeExtend($.leeUI.core.UIComponent, {
		__getType: function() {
			return 'TreeListView';
		},
		__idPrev: function() {
		...
		_render: function() {
			var g = this,
				p = this.options;
			g.header = $('<div class="lee-list-header"></div>');
			g.headerWrap = $('<div class="lee-list-header-wrap"></div>');
			g.wrap = $("<div class='lee-list-main'></div>");
			g.dataWrap = $("<div class='lee-list-list'></div>");
			g.wrap.append(g.dataWrap);

			g.treedataindex = 0;

			g._addTreeDataIndexToData(p.data);
			$(g.element).append(g.header.append(g.headerWrap)).append(g.wrap);
			g.initHeader();
			g.initView();
			g.setStyle(this.options.columns);
			g.setRowOdd();
			g.bind();
			g.wrap.bind("scroll.list", function() {
				var scrollLeft = g.wrap.scrollLeft();

				if(scrollLeft != null)
					g.header[0].scrollLeft = scrollLeft;
			});
			//g.set(p);
		},
		initHeader: function() {
			...
			$.each(cols, function(i, obj) {
				var colname = obj["text"];
				var $col = $('<div class="header col' + i + '">' + colname + '<span></span></div>').data("column", obj);

				g.headerWrap.append($col);
				g.headerWrap.append('<div class="resize" data-col=' + i + '></div>');
			});
			g.headerWrap.append('<div style="clear:both"></div>');
		},
		initView: function(data) {
			...
			$.each(data, function(i, obj) {
				data[i]["_level"] = 0;
				listArr.push(g.getChildHtml(obj, 0));
			});
			g.dataWrap.html(listArr.join(""));
		},
		getChildHtml: function(rowobj, level) {
			var g = this;
			var html = [];
			html.push(g.getView(rowobj, level));
			if(rowobj.children) {
				html.push("<div class='children-item " + (rowobj.collapse ? "hidden" : "") + "'>")
				$.each(rowobj.children, function(i, obj) {
					rowobj.children[i]["_level"] = level + 1;
					html.push(g.getChildHtml(obj, level + 1));
				});
				html.push("</div>")
			}
			return html.join("");
		...
		getView: function(rowobj, level) {
			var g = this,
				p = this.options;
			var html = [];
			html.push('<div data-treeindex="' + rowobj.treedataindex + '" class="list-item">');

			html.push('<div class="item-select"><div class="item-check fa fa-check"></div></div>')
			html.push('<div class="item-menu"><div class="cert"></div></div>');

			var cols = this.options.columns;
			$.each(cols, function(i, obj) {
				html.push('<div class="row col' + i + '">');
				...
				if(i == 0) {
					...
					html.push('<span class="arrow ' + (rowobj.collapse ? "" : "on") + '">');
					if(rowobj.children) {

						html.push('<i class="' + (rowobj.collapse ? CLS_ICON_COLLAPSE : CLS_ICON_OPEN) + '"></i>');
					}
					html.push('</span>');
					if(p.showIcon) {
						html.push('<div class="ico folder"  ><i class="x-item-file small ' + g.getIcon(rowobj) + '"></i></div>')
					}
				}
				html.push('<span class="text">' + rowobj[obj.name] + '</span>');
				html.push('</div>');
			});
		...
		appendChild: function($ele, data, sourceData, isAppend) {
			//追加子节点
			var g = this,
				p = this.options.data;
			var $childDiv = $ele.next(); //找到子节点
			if(!$childDiv.hasClass("children-item"))
				$childDiv = $("<div class='children-item'></div>").after($ele);
			var listArr = [];
			g._addTreeDataIndexToData(data); //设置treeindex

			sourceData.children = sourceData.children || [];
			if(!isAppend) {
				sourceData.children = data;
			}
			$.each(data, function(i, obj) {
				obj["_level"] = sourceData["_level"] + 1;
				listArr.push(g.getChildHtml(obj, sourceData["_level"] + 1));
				if(isAppend) {
					sourceData.children.push(obj);
				}
			});
			$childDiv[isAppend ? "append" : "html"](listArr.join(""));
		...
		bindArrow: function() {
			...
			$(this.element).on("click", ".list-item>.row>.arrow>i", function(e) {
				var $item = $(e.target).closest(".list-item");
				var treeindex = $item.attr("data-treeindex");

				var data = g._getDataByTreeDataIndex(p.data, treeindex);

				if($(e.target).parent().hasClass("on")) {
					$(e.target).parent().removeClass("on");
					//收起 触发收起事件

					$(e.target).removeClass(CLS_ICON_OPEN).addClass(CLS_ICON_COLLAPSE);
				} else {
					//展开
					// 触发展开事件

					$(e.target).parent().addClass("on");
					$(e.target).removeClass(CLS_ICON_COLLAPSE).addClass(CLS_ICON_OPEN);

					if(p.async) {
						if(p.onTreeExpand) {
							p.onTreeExpand(data, $item, g);
						}
						//g.trigger('TreeExpand', [data, $item]);
						//$(e.target).closest(".list-item").addClass("loading-children");
					}
				}
				var children = $item.next().toggleClass("hidden");
		...
		bindResize: function() {
			...
			$(".resize").drag({
				start: function(e) {
					this.index = $(e.target).attr("data-col");
					this.initDragWidth = g.options.columns[this.index].width;
				},
				move: function(e, x) {

					var cols = g.options.columns;
					var res = g.options.columns[this.index].width;
					g.options.columns[this.index].width = this.initDragWidth + x;
					g.setStyle(g.options.columns);
				},
				end: function(e, x) {
					g.options.columns[this.index].width = this.initDragWidth + x;
				}
		...
		bindHover: function() {
			$(this.element).on("mouseover", ".list-item", function(e) {
				$(this).addClass("hover")
			});

			$(this.element).on("mouseout", ".list-item", function(e) {
		...
		bindSort: function() {
			var g = this,
				p = this.options;
			$(this.element).on("click", ".header", function(e) {
				var col = $(this).data("column");

				if(col.sort) {
					$(".header.active span").html("");
					...
					if($(this).hasClass("down")) {
						...
						// 升序
						$(this).find("span").html("<i class='lee-ion-arrow-up-b'></i>");
						sortType = "asc";
					} else {
						$(this).addClass("down");
						//降序
						$(this).find("span").html("<i class='lee-ion-arrow-down-b'></i>");
						sortType = "desc";
					}
					g.sortData(col.name);
					p.sortName = col.name;
		...
		sortData: function(columnName) {
			...
			if(p.sortName == columnName) {
				//p.data.reverse();
				g._compareChildReverse(p.data);

			} else {
				p.data.sort(function(data1, data2) {
					return g._compareData(data1, data2, columnName);
				});
			}
			return p.data;
		},
		_compareChildReverse: function(data) {
			for(var i in data) {
				if(data[i].children) {
					this._compareChildReverse(data[i].children);
				}
			}
			data.reverse();
		},
		_compareData: function(data1, data2, columnName) {
			...
			if(data1.children) {
				...
				data1.children.sort(function(d1, d2) {
					return g._compareData(d1, d2, columnName);
				});
			}
			var val1 = data1[columnName],
				val2 = data2[columnName];
		...
		_getDataByTreeDataIndex: function(data, treedataindex) {
			var g = this,
				p = this.options;
			for(var i = 0; i < data.length; i++) {
				if(data[i].treedataindex == treedataindex)
					return data[i];
				if(data[i].children) {
					var targetData = g._getDataByTreeDataIndex(data[i].children, treedataindex);
					if(targetData) return targetData;
				}
			}
			return null;
		...
		bindClick: function() {
			...
			var g = this,
				p = this.options;
			$(this.element).delegate(".list-item", "click", function(e) {
				if(!g.isDragging) {
					$(".list-item.select").removeClass("select");
		...
		bindSelect: function() {
			...
			$(".lee-list-main").css("user-select", "none");
			$(".lee-list-main").mousedown(function(e) {
				r = $(this).outerWidth();
				l = $(this).find(".lee-list-list").outerHeight();
				p = $(this);
				var s = p.offset().left - p.scrollLeft();
				var o = p.offset().top - p.scrollTop();

				j = e.pageX - s;
				i = e.pageY - o;
				/*创建选择区域*/
				var c = setTimeout(function() {
					if(0 == $(".select-container").length) {
						$('<div class="select-container"></div>').appendTo($(".lee-list-main"));
						n = $(".select-container");
					}
				}, 1)
			...
			var getSelect = function() {
				...
				$.each(list, function(i, ele) {
					...
					if(Math.abs((R + L) / 2 - (nR + nL) / 2) < (R + nR - L - nL) / 2 && Math.abs((T + B) / 2 - (nT + nB) / 2) < (B + nB - T - nT) / 2) {
						$ele.addClass("select");
						console.log("拖动区域:" + nL + "-" + nT + "-" + nR + "-" + nB);
						console.log("列表区域" + L + "-" + T + "-" + R + "-" + B);
					} else {
			...
			var setScroll = function(e, t, a, i) {
				var n = i.outerHeight(),
					o = i.scrollTop(),
					s = o;
				if(e > t) {
		...
		bindDrag: function() {
			...
			$(".list-item").leeDrag({
				...
				proxyY: 0,
				proxy: function(draggable, e) {
					var proxy = $('<div class="file draggable-dragging"   id="drag-action-clear"><div class="drag-number">1</div><span><i class="font-icon fa fa-cog bg-default icon-ok"></i>1 文件</span></div>').appendTo('body');
					return proxy;
				},
				onRevert: function() {
					return false;
				...
				onStartDrag: function(current, e) {
					g.isDragging = true;
					$(".list-item.select").addClass("dragging")
				},
				onDrag: function(current, e) {
					if(tmp != "")
						this.proxy.html(tmp);
					//else
					//this.proxy.html("1个文件");
				},
				onStopDrag: function(current, e) {
					g.isDragging = false;
					$(".list-item.select").removeClass("dragging")
				}
			});
		...
		setRowOdd: function() {
			var g = this;
			var e = 0;
			$(".list-item", $(g.element)).each(function() {

				if(0 == $(this).parents(".hidden").length) {
					if(0 == e % 2) {
						$(this).addClass("alt")
					} else {
		...
		setStyle: function(colSizeArr) {
			var cssText = [];
			var width = 0;
			$.each(colSizeArr, function(i, obj) {
				cssText.push(".lee-list-header .lee-list-header-wrap .header.col" + i + "{width:" + obj.width + "px}");
				cssText.push(".lee-list-main .lee-list-list .list-item .row.col" + i + "{width:" + obj.width + "px}");
				width += obj.width;

			});

			cssText.push(".lee-list-main .lee-list-list .list-item{width:" + width + "px}");

			$.setStyle(cssText.join(""), "lee-list-col-resize");
		}
	});
})(jQuery);
	...
	$.fn.drag = function(obj, isStopPP) {
		this.each(function() {
			...
			if(isWap()) {
				...
				$that.unbind('mousedown').bind('mousedown', function(e) {
			...
			var dragMove = function(e) {
				...
				if(typeof(obj["move"]) == 'function') {
					...
					obj["move"](e, offsetX, offsetY, e, $that);
				}
			};
			var dragEnd = function(e) {
				...
				if(typeof(obj["end"]) == 'function') {
					//var mouse = getEvent(e);
					obj["end"](e, offsetX, offsetY, $that);
				}
			};
		});
	};
