...
Format.prototype.refresh = function () {
    ...
    if (graph.isSelectionEmpty()) {
        ...
        mxUtils.write(label4, "事件");
        div.appendChild(label4);


        var eventPanel = div.cloneNode(false);
        eventPanel.style.display = 'none';
        this.panels.push(new EventFormatPanel(this, ui, eventPanel));
        this.container.appendChild(eventPanel);



        addClickHandler(label2, textPanel, idx++);
        addClickHandler(label3, arrangePanel, idx++);
        addClickHandler(label4, eventPanel, idx++);
    }
};
 ...
 * Adds the label menu items to the given menu and parent.
 */
EventFormatPanel = function (format, editorUi, container) {
    BaseFormatPanel.call(this, format, editorUi, container);
    this.init();
};

mxUtils.extend(EventFormatPanel, BaseFormatPanel);

/**
 * Adds the label menu items to the given menu and parent.
 */
EventFormatPanel.prototype.init = function () {
    this.container.style.borderBottom = 'none';
    this.addEventName(this.container);
};

/**
 * Adds the label menu items to the given menu and parent.
 */
EventFormatPanel.prototype.addEventName = function (container) {
    var ui = this.editorUi;
    var editor = ui.editor;
    var graph = editor.graph;
    var ss = this.format.getSelectionState();

    //var title = this.createTitle("编码：");
    //title.style.paddingLeft = '18px';
    //title.style.paddingTop = '10px';
    //title.style.paddingBottom = '6px';
    //container.appendChild(title);
    ...
    colorPanel.style.paddingTop = '6px';
    colorPanel.style.paddingBottom = '6px';
    colorPanel.style.borderBottom = '0px';
    ...
    colorPanel.appendChild(label2);
    colorPanel.appendChild(input);
    container.appendChild(colorPanel);

    var codeidPanel = this.createPanel();
    codeidPanel.style.marginTop = '8px';
    codeidPanel.style.paddingTop = '6px';
    codeidPanel.style.paddingBottom = '6px';
    ...
    codeidPanel.appendChild(label3);
    codeidPanel.appendChild(input3);
    container.appendChild(codeidPanel);


    var panel2 = this.createPanel();
    panel2.style.marginTop = '8px';
    panel2.style.paddingTop = '6px';
    panel2.style.paddingBottom = '6px';
    panel2.style.borderBottom = '0px';
    ...
    panel2.appendChild(label1);
    panel2.appendChild(btnset);
    ...
    var eventPanel = this.createPanel();
    eventPanel.style.marginTop = '8px';
    //colorPanel.style.borderTop = '1px solid #c0c0c0';
    eventPanel.style.paddingTop = '6px';
    eventPanel.style.paddingBottom = '6px';

    var input2 = document.createElement("textarea");
    input2.style = "width:200px;height:250px;"
    input2.id = "clickevent"



    eventPanel.appendChild(input2);
    if (ss.vertices.length == 1) {
        input.value = ss.vertices[0].id;
        //if (Page.Config.General.GraphConfig.length > 0) {
        //    for (var i = 0; i < Page.Config.General.GraphConfig.length; i++) {
        //        if (Page.Config.General.GraphConfig[i].id == input.value) {
        //            input2.value = Page.Config.General.GraphConfig[i].onclick;
        //            input3.value = Page.Config.General.GraphConfig[i].onclickparam;
        //            break;
        //        }
        //    }
        //}
    } else if (ss.edges && ss.edges.length == 1) {
        input.value = ss.edges[0].id
    }
    ...
    var updateid = function () {
        ...
        if (cell && input.value && cell.id != input.value) {
            var oldvalue = cell.id;
            var newvalue = input.value;
            if (input.value == "1" || input.value == "0") {
                input.value = oldvalue;
                return alert("编码不能设置为0和1；0和1为系统占用编码");
            }
            ...
            if (Page.Config.General.GraphConfig.length > 0) {
                ...
                var ishavenew = false;
                for (var i = 0; i < Page.Config.General.GraphConfig.length; i++) {
                    if (Page.Config.General.GraphConfig[i].id == newvalue) {
                        ishavenew = true;
                        break;
                    }
                }
                if (!ishavenew) {
                    for (var i = 0; i < Page.Config.General.GraphConfig.length; i++) {
                        if (Page.Config.General.GraphConfig[i].id == oldvalue) {
                            ishaveOld = true;
                            Page.Config.General.GraphConfig[i].id = newvalue;
                            break;
                        }
                    }
                    if (!ishaveOld) {
                        Page.Config.General.GraphConfig.push({ id: newvalue, onclick: "", onclickparam: "" })
                    }
                    cell.id = input.value;
                }
                else {
                ...
                Page.Config.General.GraphConfig.push({ id: input.value, onclick: "", onclickparam: "" })
                cell.id = input.value;
            }
    ...
    var addCellProperty = function () {
        if (input3.value) {
            var ishave = false;
            for (var i = 0; i < Page.Config.General.GraphConfig.length; i++) {
                if (Page.Config.General.GraphConfig[i].id == input.value) {
                    ishave = true;
                    Page.Config.General.GraphConfig[i].onclickparam = input3.value;
                    break;
                }
            }
            if (!ishave) {
                Page.Config.General.GraphConfig.push({ id: input.value, onclick: "", onclickparam: input3.value })
            }
            //var value = graph.getModel().getValue(ss.vertices[0]);
            ...
            //var oldvalue ="";
            //var newvalue = input3.value;
            //if(value.attributes.code)
            //    oldvalue = value.attributes.code.nodeValue;
            //if (Page.Config.General.GraphConfig.length > 0) {
            //    var ishaveOld = false;
            //    var ishavenew = false;
            //    for (var i = 0; i < Page.Config.General.GraphConfig.length; i++) {
            //        if (Page.Config.General.GraphConfig[i].onclickparam == newvalue) {
            //            ishavenew = true;
            //            break;
            //        }
            //    }
            ...
            //            if (Page.Config.General.GraphConfig[i].id == input.value) {
            //                ishaveOld = true;
            //                Page.Config.General.GraphConfig[i].onclickparam = newvalue;
            //                break;
            //            }
            //        }
            //        if (!ishaveOld) {
            //            Page.Config.General.GraphConfig.push({ id: input.value, onclick: "", onclickparam: input3.value })
            //        }
            //        value.setAttribute('code', input3.value);
            //        graph.getModel().setValue(ss.vertices[0], value);
            //    }
            //    else {
            //        input3.value = oldvalue;
            //        alert(newvalue + "已经存在.");
            //    }
            //}
            //else {
