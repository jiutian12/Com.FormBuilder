...
namespace FormBuilder.Web.Controllers
{
    public class DataModelController : Controller
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GetObjects(string pagesize, string page, string keword)
        {
            try
            {
                ...
                long total = 0;
                long totalpage = 0;
                var list = this._service.GetObjects(keword, int.Parse(page), int.Parse(pagesize), out totalpage, out total);
                return Json(list);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public string getModelTreeData(string modelID, string level, string path, string parentID, string keyWord, string filter, string order)
        {
            try
            {
                var data = this._service.getModelTreeDataDictList(modelID, level, path, parentID, keyWord, filter, order);
                DataTable dt = new DataTable();
                return Newtonsoft.Json.JsonConvert.SerializeObject(new { res = true, data = data });
            }
            catch (Exception ex)
